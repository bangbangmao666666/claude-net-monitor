...
- **`clash.sh`**：Clash(mihomo) API 操作模块（被 monitor.sh 调用），负责候选节点的列举、测速与切换。

## 自动切换候选节点（可选，默认开启）

代理出境持续变差时，自动在所有**候选节点**（默认日本）之间切到延迟最低的健康节点——只在候选节点间切，
绝不切到候选外的地区（固定日本可防某些服务因地区跳变封号）。
...
候选节点的识别在 `clash.sh` 顶部 `NODE_KEYWORDS`（默认 `日本`）配置；多个地区用 `|` 分隔，如 `日本|香港`。
改此常量后重新 `install.sh` 会覆盖、需重改，或用环境变量 `NODE_KEYWORDS` 覆盖。
